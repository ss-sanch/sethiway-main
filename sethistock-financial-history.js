            ...
            html += `
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 flex flex-col min-h-[350px]">
                    <div class="flex items-center justify-between gap-3 mb-2">
                        ...
                        <span class="px-2 py-1 rounded-md bg-gray-50 border border-gray-100 text-[9px] font-black text-gray-400 uppercase tracking-widest">${periodLabel}</span>
                    </div>
                    <div id="${card.id}" class="h-56 w-full mt-auto mb-2"></div>
                    <div class="grid grid-cols-4 gap-2 mt-4 pt-4 border-t border-gray-100">
                        <div class="text-center"><p class="text-[9px] text-gray-400 font-bold uppercase">1Y CAGR</p><p class="font-semibold text-xs ${growthColour(growth['1Y'])}">${growth['1Y']}</p></div>
                        <div class="text-center"><p class="text-[9px] text-gray-400 font-bold uppercase">2Y CAGR</p><p class="font-semibold text-xs ${growthColour(growth['2Y'])}">${growth['2Y']}</p></div>
                        <div class="text-center"><p class="text-[9px] text-gray-400 font-bold uppercase">3Y CAGR</p><p class="font-semibold text-xs ${growthColour(growth['3Y'])}">${growth['3Y']}</p></div>
                        <div class="text-center"><p class="text-[9px] text-gray-400 font-bold uppercase">MAX</p><p class="font-semibold text-xs ${growthColour(growth.MAX)}">${growth.MAX}</p></div>
                    </div>
                </div>`;
        });
